...
The realtime tutor loop is verified end to end: backend tests are green, frontend verify is green, and the browser smoke suite is green. The session flow now covers typed turns, interruption, local-only avatar selection, history continuity, reload continuity, and timed phoneme lip sync. The shipped browser path is no longer relying on a hand-wavy claim that the avatar should move. A browser regression samples the GLB morph influences and requires at least three distinct rendered visemes in one utterance.
...
The docs are now aligned to the UI that actually shipped. The live tutor interaction is on `/session`. The self-hosted TalkingHead tutor is the only selectable choice on `/avatar`; managed provider integrations remain internal. The real session controls are `Send`, `Hold to talk`, `History`, `New`, and `Escape` for interruption. The old language about `Send Text Turn`, an on-screen `Interrupt` button, or subject and grade selectors on the session page is no longer the source of truth.
